...
* AOE (Activity On Edges)网络 ：如果在无有向环的带权有向图中用**<u>有向边表示一个工程中的各项活动</u>**(Activity)，用边上的**<u>权值表示活动的持续时间</u>**(Duration)， 用**<u>顶点表示事件</u>**(Event)，则这样的有向图叫做用边表示活动的网络，简称AOE (Activity On Edges)网络。AOE网是一个带权的有向 无环图。
...
* 首先了解一些术语
  ...
  最早开始时间(ES)指某项活动最早能够开始的时间。

  最迟开始时间 (LS) 指为了某项活动能够在要求的时间节点完成，最迟必须开始的时间 。

  最早结束时间 (EF) 指某项活动最早能够完成的时间。

  最迟结束时间(LF) 指为了某项活动能够在要求的时间节点完成，最迟必须完成的的时间。

  总时差(TS):项目EF和项目要求完成时间的差值,也成为浮动量(float)

* 下面介绍求关键路径的另一种算法
  * 关键活动都是不允许拖延的活动，因此这些活动es=ls，因此求关键路径问题转化为求es==ls的那些活动的总时长（路径的总长度）问题，也就是求es[]和ls[]数组问题
  * 对于Vi----Ar------Vj，活动Ar的最早开始时间等于事件Vi的最早结束时间，最迟开始事件等于Vj的最迟开始事件减去Ar的时间；因此问题转化为
