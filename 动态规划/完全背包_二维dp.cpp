...
int dp[501][maxn]; //dp[i][j]表示总体积**恰好**为j时，前i个物品能达到的最大价值
int main(){
    int n;
    cin>>n;
    for(int q=0;q<n;q++){
        ...
        for(int i=1;i<=m;i++){ //遍历每一个物品
            ...
            for(int j=data[i].w;j<=t;j++){ //必须顺序遍历，因为需要的已知值有d[i][]的
                ...
                dp[i][j] = min(dp[i-1][j],dp[i][j-data[i].w]+data[i].v); //题目要求价值最小
            }
        }
        // for(int i=0;i<=m;i++){
        //     for(int j=0;j<=t;j++){
        ...
        // }
        if(dp[m][t]<inf)
            cout<<"The minimum amount of money in the piggy-bank is "<<dp[m][t]<<"."<<endl;
